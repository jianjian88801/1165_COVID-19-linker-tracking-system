        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '开始去的时间' : ''">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:ss:mm"
                                v-model="searchForm.guijiTimeStartSearch"
                                type="datetime"
                                placeholder="开始时间">
                        </el-date-picker>
                    </el-form-item>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '结束去的时间' : ''">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:ss:mm"
                                v-model="searchForm.guijiTimeEndSearch"
                                type="datetime"
                                placeholder="结束时间">
                        </el-date-picker>
                    </el-form-item>
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '地点名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.didianName" placeholder="地点名称" clearable></el-input>
                    </el-form-item>
                     
                    <el-form-item :label="contents.inputTitle == 1 ? '地点状态' : ''">
                        <el-select v-model="searchForm.didianTypes" placeholder="请选择地点状态">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('guiji','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiechuzhe/upload/guijiMuBan.xls"
                        >批量导入用户轨迹数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('guiji','导入导出')"
                                ...
                                :on-success="guijiUploadSuccess"
                                :on-error="guijiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('guiji','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入用户轨迹数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('guiji','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "guiji.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('guiji','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('guiji','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('guiji','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.didianName!= '' && this.searchForm.didianName!= undefined) {
                    params['didianName'] = '%' + this.searchForm.didianName + '%'
                }
                ...
                if (this.searchForm.guijiTimeStartSearch != '' && this.searchForm.guijiTimeStartSearch != undefined) {
                    params['guijiTimeStart'] = this.searchForm.guijiTimeStartSearch
                }
                if (this.searchForm.guijiTimeEndSearch != '' && this.searchForm.guijiTimeEndSearch != undefined) {
                    params['guijiTimeEnd'] = this.searchForm.guijiTimeEndSearch
                }
                
                params['guijiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            guijiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "guiji/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
